...
export default function ServiceCard({ title, body, icon, link }: Props){
  return (
    <motion.article
      ...
      className="bg-white rounded-2xl p-8 shadow-[0_12px_30px_rgba(27,67,50,0.08)] border border-sand transition-all duration-300"
    >
      <div className="w-12 h-12 bg-cream border border-sand rounded-xl flex items-center justify-center text-forest">{icon}</div>
      <h3 className="font-display text-2xl text-forest mt-4 mb-2">{title}</h3>
